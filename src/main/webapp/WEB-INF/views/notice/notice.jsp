	...
	<div class="container">
		...
		<div class="container table-responsive w-100">
			<table class="table">
				<thead>
					<tr>
						<td colspan="5">알바정보 | 총 ${totalcnt }개</td>
					</tr>
					<tr>
						<td colspan="5">등록순<strong><a href="ordercheck?ot=1">∨</a><a
					...
					<c:forEach items="${list }" var="dto">
						<tr>
							...
							<td><a href="content_view?c=${dto.nseq }&p=${memberID }">${dto.ntitle }</a>
								...
								</c:if> <c:if test="${not empty sessionScope.preview }">
								...
								<tbody>
									<tr height="50px">
										<td colspan="5"><strong>업 직 종</strong>&nbsp;&nbsp;&nbsp;
											${dto.ncategory } <br /> <strong>기간·요일</strong>
											${dto.nperiod }|${dto.nday } <strong>마 감 일</strong> <c:if
												test="${empty dto.ndeadline }">상시모집</c:if> <c:if
												test="${not empty dto.ndeadline}">${dto.ndeadline } (${lefttime })</c:if>
										</td>
									</tr>
								</tbody>
							</c:if>
